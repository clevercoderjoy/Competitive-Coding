...
#exception handling to avoid NZEC error
try:
    #taking inputs as per the question and maping them to float to obtain output up to 2 decimal places
    amount_to_be_withdrawn, balance = map(float,input().split())
    #checking the conditions
    if (amount_to_be_withdrawn % 5 == 0) and amount_to_be_withdrawn + 0.50 <= balance:
        #updating the balance
        balance = balance - (amount_to_be_withdrawn + 0.50)
